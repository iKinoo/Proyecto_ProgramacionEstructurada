...
float disminuir_matriz(float **matriz, int filas, int columnas, int limite, float escalar)
{
    ...
    {
        cofactor = pow(-1, limite) * escalar * determinante_sarrus_matriz(submatriz, 1);
...
{
    float det_final = 0;
    // EnvÃ­amos un valor de la fila elegida
...
void inversaGaussJordan(float **matriz_a)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        printf("Divisi�n de la fila %d por el valor de la diagonal:\n", i);
        imprimir_matriz(matriz_a, n, m);
        imprimir_matriz(matriz_b, n, m);

        // se realiza la eliminaciÃ³n hacia adelante
        for (int j = i + 1; j < n; j++)
        {
        ...
        printf("Eliminaci�n hacia adelante (fila %d eliminada de las filas siguientes):\n", i);
        imprimir_matriz(matriz_a, n, m);
        imprimir_matriz(matriz_b, n, m);
    }

    // se realiza la eliminaciÃ³n hacia atrÃ¡s
    for (int i = n - 1; i >= 0; i--)
    {
        for (int j = i - 1; j >= 0; j--)
        {
        ...
        printf("Eliminaci�n hacia atr�s (fila %d eliminada de las filas anteriores):\n", i);
        imprimir_matriz(matriz_a, n, m);
        imprimir_matriz(matriz_b, n, m);
    }
